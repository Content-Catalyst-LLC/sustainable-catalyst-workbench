#!/usr/bin/env bash
set -Eeuo pipefail
VERSION="8.2.0"; PRODUCT="Workbench"; ARCHIVE="${1:-/tmp/sustainable-catalyst-workbench-backend-v8.2.0.zip}"
...
TMP="$(mktemp -d /tmp/sc-workbench-v820.XXXXXX)"; trap 'rm -rf "$TMP"' EXIT
...
marker="$(find "$TMP/package" -type f -path '*/backend/app/v820.py' | head -1)"; [[ -n "$marker" ]] || fail "v8.2.0 project-workspace module missing from package"
SRC_BACKEND="$(dirname "$(dirname "$marker")")"; PACKAGE_ROOT="$(dirname "$SRC_BACKEND")"
grep -q 'APP_VERSION = "8.2.0"' "$SRC_BACKEND/app/release.py" || fail "package version mismatch"
grep -q 'sc-workbench-unified-research-project-workspace/1.0' "$SRC_BACKEND/app/v820.py" || fail "project workspace schema missing"
grep -q 'sustainable-catalyst-workbench:8.2.0' "$PACKAGE_ROOT/compose.yml" || fail "compose release identity mismatch"
...
rsync -a --exclude='data/' --exclude='__pycache__/' --exclude='.pytest_cache/' --exclude='*.pyc' --exclude='.env' --exclude='.env.*' "$SRC_BACKEND/" "$LIVE_BACKEND/"; cp "$PACKAGE_ROOT/compose.yml" "$COMPOSE"; [[ -f "$PACKAGE_ROOT/workbench-v8.2.0.env.example" ]] && cp "$PACKAGE_ROOT/workbench-v8.2.0.env.example" "$ROOT/workbench-v8.2.0.env.example"
...
echo "=== VERIFYING UNIFIED RESEARCH PROJECT WORKSPACE ==="
...
for path in ('/health','/v820/status','/research-projects/manifest'):
 s,b=req(path); assert s==200 and b.get('ok') is True,(path,b)
health=req('/health')[1]; assert health['version']=='8.2.0',health
uid=uuid.uuid4().hex[:12]; envkey='deploy-v820-env-'+uid; projectkey='deploy-v820-project-'+uid
spec={'environmentKey':envkey,'title':'Deployment Project Environment','projectEntityId':projectkey,'components':[{'componentKey':'notebook','componentType':'notebook-run','payload':{'notebookRunHash':'deploy-nb','cellRuns':[]}}]}
s,env=req('/research-environment/build',{'environment':spec}); assert s==200,env
s,saved_env=req('/research-environment/persistence/save',{'researchEnvironment':env,'expectedCurrentRevision':0,'reason':'deployment-verification'}); assert s==200 and saved_env['revision']==1,saved_env
s,ws=req('/research-projects/build',{'project':{'projectKey':projectkey,'title':'Deployment Project','activeEnvironmentKey':envkey,'activeEnvironmentRevision':1}}); assert s==200 and ws['dashboard']['componentCount']==1,ws
...
s,loaded=req('/research-projects/'+urllib.parse.quote(projectkey,safe='')); assert s==200 and loaded['workspace']['workspaceHash']==ws['workspaceHash'],loaded
s,activity=req('/research-projects/'+urllib.parse.quote(projectkey,safe='')+'/activity'); assert s==200 and activity['activityCount']==1,activity
s,surface=req('/research-projects/surface/plan',{'projectKey':projectkey,'surface':'notebook','action':'inspect'}); assert s==200 and surface['scientificExecutionPerformed'] is False,surface
print('PASS: /health reports Workbench 8.2.0')
print('PASS: durable project-centric workspace save/load/listing and activity history are active')
print('PASS: project dashboards resolve the active v8.1 persisted research environment')
print('PASS: project surface navigation planning is explicit and non-executing')
print('PASS: v8.1 environment revision history remains authoritative and project persistence does not rewrite it')
PYVERIFY
