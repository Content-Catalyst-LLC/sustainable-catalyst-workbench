#!/usr/bin/env bash
set -Eeuo pipefail
VERSION="7.10.0"; PRODUCT="Workbench"; ARCHIVE="${1:-/tmp/sustainable-catalyst-workbench-backend-v7.10.0.zip}"
...
TMP="$(mktemp -d /tmp/sc-workbench-v7100.XXXXXX)"; trap 'rm -rf "$TMP"' EXIT
...
marker="$(find "$TMP/package" -type f -path '*/backend/app/v7100.py' | head -1)"; [[ -n "$marker" ]] || fail "v7.10.0 Interactive Computational Notebook Runtime module missing from package"
SRC_BACKEND="$(dirname "$(dirname "$marker")")"; PACKAGE_ROOT="$(dirname "$SRC_BACKEND")"
grep -q 'APP_VERSION = "7.10.0"' "$SRC_BACKEND/app/release.py" || fail "package version mismatch"
grep -q 'sc-workbench-interactive-computational-notebook/1.0' "$SRC_BACKEND/app/v7100.py" || fail "workflow graph schema missing"
grep -q 'sc.research.workflow-orchestration.v1' "$SRC_BACKEND/app/v7100.py" || fail "Core workflow contract missing"
grep -q 'sustainable-catalyst-workbench:7.10.0' "$PACKAGE_ROOT/compose.yml" || fail "compose release identity mismatch"
...
rsync -a --exclude='data/' --exclude='__pycache__/' --exclude='.pytest_cache/' --exclude='*.pyc' --exclude='.env' --exclude='.env.*' "$SRC_BACKEND/" "$LIVE_BACKEND/"; cp "$PACKAGE_ROOT/compose.yml" "$COMPOSE"; [[ -f "$PACKAGE_ROOT/workbench-v7.10.0.env.example" ]] && cp "$PACKAGE_ROOT/workbench-v7.10.0.env.example" "$ROOT/workbench-v7.10.0.env.example"
...
echo "=== VERIFYING INTERACTIVE COMPUTATIONAL NOTEBOOK RUNTIME ==="
...
for path in ('/health','/runtime','/capabilities','/v7100/status','/notebooks/manifest'):
 s,b,_=req(path); assert s==200,(path,s); assert b.get('ok') is True,(path,b)
health=req('/health')[1]; assert health['version']=='7.10.0' and health['coreCompatible'] is True,health
...
s,run,_=req('/notebooks/run',{'notebook':nb}); assert s==200,run; assert run['ok'] and run['completedExecutableCellCount']==2 and run['hiddenInterpreterStateUsed'] is False,run
s,check,_=req('/notebooks/run/validate',{'notebookRun':run}); assert s==200 and check['valid'] is True,check
s,replay,_=req('/notebooks/replay/plan',{'notebookRun':run}); assert s==200 and replay['replayPerformed'] is False,replay
s,graph,_=req('/notebooks/workflow-graph/plan',nb); assert s==200 and len(graph['graph']['nodes'])==2,graph
headers={'X-Request-ID':'deploy-v7100','X-SC-Gateway-Service':'workbench','X-SC-Core-Version':os.getenv('SCWB_CORE_EXPECTED_VERSION_PREFIX','3.')}
...
s,plan,_=req('/integration/core/notebook-workflow/plan',{'notebookRun':run,'coreWorkflowId':'core-nb-v7100'},headers); assert s==200,plan; assert plan['coreWorkflowContract']=='sc.research.workflow-orchestration.v1' and len(plan['stageRegistrations'])==2 and plan['automaticCoreDispatchAuthorized'] is False,plan
print('PASS: /health reports Workbench 7.10.0')
print('PASS: typed computational notebook cells and explicit dependency bindings are active')
print('PASS: notebook-run integrity validation and replay planning are active with no hidden interpreter state')
print('PASS: notebook projection to the v7.9 scientific workflow graph is active')
print('PASS: Platform Core notebook workflow planning remains two-phase and non-dispatching')
PYVERIFY
