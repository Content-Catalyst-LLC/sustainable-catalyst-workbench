...
def test_manifest_status_capabilities():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='8.1.0' and d['capabilities']['referenceBenchmarkEvaluation']
    s=cl.get('/v780/status').json(); assert s['ok'] and s['datasetComparison'] and s['automaticModelAcceptance'] is False
    caps=cl.get('/capabilities').json(); assert caps['version']=='8.1.0' and caps['coreIntegration']['modelValidationVerificationFramework'] is True
