...
def test_manifest_status_capabilities():
    m=c.get('/comparative-analysis/manifest').json(); assert m['ok'] and m['version']=='9.0.0' and m['capabilities']['pairwiseNumericDeltas']
...
def test_core_plan_is_neutral_and_two_phase(monkeypatch,tmp_path):
    ...
    b=r.json(); assert b['version']=='9.0.0' and b['comparisonBindingIsAnalyticalViewOnly'] is True
