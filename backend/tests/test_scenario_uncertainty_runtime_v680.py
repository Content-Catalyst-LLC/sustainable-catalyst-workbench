...
def test_service_token_policy_applies_to_v680(monkeypatch):
    ...
    assert ok.status_code==200 and ok.json()['version']=='9.1.0'

def test_v680_status_and_capabilities():
    c=client(); s=c.get('/v680/status').json(); assert s['ok'] is True and s['version']=='9.1.0' and s['sobol'] is True
    caps=c.get('/capabilities').json(); assert caps['version']=='9.1.0'; assert caps['coreIntegration']['scenarioUncertaintyRuntime'] is True
