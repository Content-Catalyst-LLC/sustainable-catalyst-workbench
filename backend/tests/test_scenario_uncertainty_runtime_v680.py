...
def test_v680_status_and_capabilities():
    c=client(); s=c.get('/v680/status').json(); assert s['ok'] is True and s['version']=='7.2.0' and s['sobol'] is True
    caps=c.get('/capabilities').json(); assert caps['version']=='7.2.0'; assert caps['coreIntegration']['scenarioUncertaintyRuntime'] is True
