...
def test_manifest_status_and_capabilities():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='7.12.0' and d['capabilities']['dependencyDirectedAcyclicGraphs']
    s=cl.get('/v790/status').json(); assert s['ok'] and s['typedNodes'] and s['hiddenOutputSubstitution'] is False
    caps=cl.get('/capabilities').json(); assert caps['version']=='7.12.0' and caps['coreIntegration']['scientificWorkflowGraph'] is True
