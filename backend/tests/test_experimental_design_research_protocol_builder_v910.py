...
def test_manifest_status_and_boundaries():
    m=c.get('/protocol-builder/manifest').json(); assert m['ok'] and m['version']=='9.9.0' and m['capabilities']['formalResearchProtocolObject']
...
def test_compose_protocol_and_readiness(monkeypatch,tmp_path):
    ...
    d=r.json(); assert d['version']=='9.9.0' and len(d['protocolHash'])==64 and d['sourceHashes']['studyHash']==study
