...
def test_manifest_status_capabilities_current_identity():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='9.1.0' and d['contentAddressedDatasets'] if 'contentAddressedDatasets' in d else d['capabilities']['contentAddressedDatasets']
    s=c.get('/v730/status').json(); assert s['ok'] and s['version']=='9.1.0' and s['unitAware'] is True
    caps=c.get('/capabilities').json(); assert caps['version']=='9.1.0'; assert caps['coreIntegration']['datasetVariableParameterWorkspace'] is True
...
def test_core_route_token_boundary(monkeypatch):
    ...
    ok=c.post('/integration/core/data-workspace/lineage/plan',headers={'X-SC-Service-Token':'secret730'},json={'workspace':ws}); assert ok.status_code==200 and ok.json()['version']=='9.1.0'
