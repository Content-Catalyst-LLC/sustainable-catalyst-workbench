...
def test_manifest_catalog_status_and_capabilities():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='8.5.0' and d['solverCount']>=20
    cat=c.get('/solvers/catalog').json(); assert cat['solverCount']==d['solverCount']; assert cat['defaultSolvers']['root']=='root.brentq'
    s=c.get('/v740/status').json(); assert s['ok'] and s['version']=='8.5.0' and s['convergenceDiagnostics'] is True
    caps=c.get('/capabilities').json(); assert caps['version']=='8.5.0'; assert caps['coreIntegration']['numericalMethodsSolverRuntime'] is True
...
def test_core_lineage_plan_two_phase_and_token_boundary(monkeypatch):
    ...
    ok=c.post('/integration/core/solver-lineage/plan',headers={'X-SC-Service-Token':'secret740'},json={'solverResult':solved}); assert ok.status_code==200 and ok.json()['version']=='8.5.0'
