...
def test_manifest_and_status():
    m=c.get('/research-environment/manifest').json(); assert m['ok'] and m['version']=='9.2.0'
    ...
    s=c.get('/v800/status').json(); assert s['ok'] and s['version']=='9.2.0' and s['v7SeriesIntegrated'] is True
