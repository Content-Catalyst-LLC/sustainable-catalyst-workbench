...
def test_manifest_status_capabilities():
    m=c.get('/linked-scientific-views/manifest').json(); assert m['ok'] and m['version']=='8.7.0'
...
def test_core_plan_is_two_phase_and_view_only(monkeypatch,tmp_path):
    ...
    b=p.json(); assert b['version']=='8.7.0' and b['linkedViewBindingIsViewStateOnly'] is True
