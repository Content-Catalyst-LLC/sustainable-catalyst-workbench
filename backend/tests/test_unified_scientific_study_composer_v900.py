...
def test_manifest_status_and_boundaries():
    m=c.get('/study-composer/manifest').json(); assert m['ok'] and m['version']=='9.12.0' and m['capabilities']['topLevelScientificStudyObject']
...
def test_compose_reports_explicit_stage_readiness(monkeypatch,tmp_path):
    ...
    d=r.json(); assert d['version']=='9.12.0' and len(d['studyHash'])==64
