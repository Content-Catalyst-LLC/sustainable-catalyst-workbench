...
def test_framework_contract():
    x=framework(); assert x["ok"] is True; assert x["target_key"]=='workbench'; assert x["consumer_version"]=='6.1.0'; assert x["capabilities"]["automatic_execution"] is False; assert x["capabilities"]["persistence"] is False
