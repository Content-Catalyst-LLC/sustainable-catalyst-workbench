...
def seed_sources(project):
    ...
    uh='u'*64
    study={'ok':True,'schema':'sc-workbench-uncertainty-sensitivity-study/1.0','version':'9.9.0','projectKey':project,'studyHash':uh,'studyRef':f'sc://workbench/uncertainty-sensitivity/{project}/{uh}','studyKey':'u1','title':'Uncertainty','campaignHash':'c'*64,'samplingMethod':'monte-carlo','sampleCount':100,'createdBy':'test','createdAt':'2026-09-26T00:00:00Z'}
    ...
    kh='k'*64
    cal={'ok':True,'schema':'sc-workbench-model-calibration/1.0','version':'9.9.0','projectKey':project,'calibrationHash':kh,'calibrationRef':f'sc://workbench/model-calibration/{project}/{kh}','calibrationKey':'k1','title':'Calibration','campaignHash':'c'*64,'problemHash':'p'*64,'createdBy':'test','createdAt':'2026-09-26T00:00:00Z'}
...
def test_manifest_and_status():
    m=c.get('/results-synthesis/manifest').json(); assert m['ok'] and m['version']=='9.9.0'
