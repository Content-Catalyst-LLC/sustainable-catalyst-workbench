...
def test_manifest_status_and_capabilities():
    ...
    d=m.json(); assert d['version']=='8.8.0'; assert d['corePredictiveModelContract']=='sc.predictive.model.v1'; assert d['coreVisualPredictiveContract']=='sc.visual-runtime.predictive-intelligence.v1'; assert d['boundaries']['coreExecutesPredictiveModels'] is False
    s=c.get('/v6100/status').json(); assert s['ok'] and s['version']=='8.8.0' and s['probabilisticIntervals'] is True
...
def test_service_token_guard(monkeypatch):
    ...
    ok=c.get('/integration/core/predictive-intelligence/manifest',headers={'X-SC-Service-Token':'secret6100'}); assert ok.status_code==200 and ok.json()['version']=='8.8.0'
