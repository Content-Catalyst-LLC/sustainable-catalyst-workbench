...
def test_manifest_and_status_boundaries():
    m=c.get('/production-certification/manifest').json(); assert m['ok'] and m['version']=='9.12.0'
...
def test_production_certification_report_passes(monkeypatch,tmp_path):
    ...
    assert d['publicationHandoff']['version']=='9.12.0' and d['publicationHandoff']['evidenceManifestGeneration'] is True
