...
def test_manifest_catalog_status_and_capabilities_current_identity():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='8.6.0'
    ...
    s=c.get('/v720/status').json(); assert s['ok'] and s['version']=='8.6.0' and s['deterministicRouting'] is True
...
def test_orchestrated_execution_runs_v700_and_projects_v710_object():
    ...
    obj=d['executionObject']; assert obj['version']=='8.6.0' and obj['objectKind']=='single_execution'
...
def test_core_route_token_boundary(monkeypatch):
    ...
    assert ok.status_code==200 and ok.json()['version']=='8.6.0'
