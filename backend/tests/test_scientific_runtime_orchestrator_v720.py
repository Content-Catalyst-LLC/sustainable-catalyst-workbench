...
def test_manifest_catalog_status_and_capabilities_current_identity():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='7.6.0'
    ...
    s=c.get('/v720/status').json(); assert s['ok'] and s['version']=='7.6.0' and s['deterministicRouting'] is True
    caps=c.get('/capabilities').json(); assert caps['version']=='7.6.0'
...
def test_orchestrated_execution_runs_v700_and_projects_v710_object():
    ...
    obj=d['executionObject']; assert obj['version']=='7.6.0' and obj['objectKind']=='single_execution'
