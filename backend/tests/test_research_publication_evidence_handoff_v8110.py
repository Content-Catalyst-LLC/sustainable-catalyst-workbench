...
def test_manifest_status_capabilities():
    m=c.get('/publication-handoff/manifest').json(); assert m['ok'] and m['version']=='9.11.0'
...
def test_handoff_preserves_snapshot_narrative_and_evidence(monkeypatch,tmp_path):
    ...
    p=r.json(); assert p['version']=='9.11.0' and p['snapshotHash']==snapshot and p['packageHash']
...
def test_core_plan_is_two_phase_and_non_publishing(monkeypatch,tmp_path):
    ...
    p=r.json(); assert p['version']=='9.11.0' and p['publicationHandoffIsGovernedDerivedObjectOnly'] is True
