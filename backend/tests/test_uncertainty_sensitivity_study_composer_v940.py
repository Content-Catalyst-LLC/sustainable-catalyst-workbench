...
def test_manifest_and_status():
    m=c.get('/uncertainty-sensitivity/manifest').json(); assert m['ok'] and m['version']=='10.0.0'
...
def test_sensitivity_statistics_are_neutral(monkeypatch,tmp_path):
    ...
    campaign_hash='a'*64
    campaign={'ok':True,'schema':'sc-workbench-computational-campaign/1.0','version':'10.0.0','projectKey':p,'campaignHash':campaign_hash,'campaignRef':f'sc://workbench/campaign/{p}/{campaign_hash}','campaignKey':'c','title':'C','plannedRunCount':5,'recordLabel':'C','createdBy':'test','createdAt':'2026-09-25T00:00:00Z'}
