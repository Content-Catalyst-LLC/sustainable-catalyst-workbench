...
def write_job(tmp_path, project, jid, value, x=None, group=None):
    ...
    job={
      'ok':True,'schema':'sc-workbench-execution-job/1.0','version':'9.3.0','jobId':jid,'projectKey':project,
...
def test_manifest_status_boundaries():
    m=c.get('/statistical-workspace/manifest').json(); assert m['ok'] and m['version']=='9.3.0'
