...
def test_manifest_status():
    m=c.get('/execution-console/manifest').json(); assert m['ok'] and m['version']=='8.8.0' and m['capabilities']['durableProjectExecutionJobs'] is True
    s=c.get('/v840/status').json(); assert s['ok'] and s['version']=='8.8.0' and s['hiddenBackgroundExecution'] is False
