...
def test_manifest_status_and_capabilities():
    ...
    d=m.json(); assert d['version']=='9.8.0'; assert d['coreForensicQuantitativeContract']=='sc.open-forensics.quantitative-reconstruction.v1'; assert d['coreForensicHandoffContract']=='sc.forensic-quantitative-handoff.v1'; assert d['boundaries']['truthDeterminationAuthorized'] is False
    s=c.get('/v6110/status').json(); assert s['ok'] and s['version']=='9.8.0' and s['trajectoryReconstruction'] is True and s['hypothesisRanking'] is False
...
def test_service_token_guard(monkeypatch):
    ...
    ok=c.get('/integration/core/forensic-quantitative/manifest',headers={'X-SC-Service-Token':'secret6110'}); assert ok.status_code==200 and ok.json()['version']=='9.8.0'
