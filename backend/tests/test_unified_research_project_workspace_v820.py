...
def test_manifest_and_status():
    m=c.get('/research-projects/manifest').json(); assert m['ok'] and m['version']=='9.3.0' and m['capabilities']['projectCentricWorkspace'] is True
    s=c.get('/v820/status').json(); assert s['ok'] and s['version']=='9.3.0' and s['environmentHistoryAuthority']=='v8.1'
