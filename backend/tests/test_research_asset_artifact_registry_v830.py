...
def test_manifest_status():
    m=c.get('/research-assets/manifest').json(); assert m['ok'] and m['version']=='8.7.0' and m['capabilities']['projectScopedAssetRegistry'] is True
    s=c.get('/v830/status').json(); assert s['ok'] and s['version']=='8.7.0' and s['projectStateAuthority']=='v8.2' and s['environmentStateAuthority']=='v8.1'
