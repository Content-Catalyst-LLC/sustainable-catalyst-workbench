...
def test_manifest_status():
    m=c.get('/research-assets/manifest').json(); assert m['ok'] and m['version']=='9.2.0' and m['capabilities']['projectScopedAssetRegistry'] is True
    s=c.get('/v830/status').json(); assert s['ok'] and s['version']=='9.2.0' and s['projectStateAuthority']=='v8.2' and s['environmentStateAuthority']=='v8.1'
...
def test_capabilities_advertise_v830():
    caps=c.get('/capabilities').json(); assert caps['version']=='9.2.0'
