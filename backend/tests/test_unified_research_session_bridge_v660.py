...
def test_project_session_binding_builds_product_project_object_and_variable_bindings():
 x=build_project_session_bindings(ProjectSessionBindRequest(coreSessionId="session-core-1",project=sample_project())); assert x["ok"] is True and x["coreSessionId"]=="session-core-1"; assert x["productBinding"]["product_ref"]==PRODUCT_REF and x["productBinding"]["product_version"]=="7.11.0"; types=[b["object_type"] for b in x["objectBindings"]]; assert "workbench.computational-project" in types and "workbench.computational-object" in types and "workbench.shared-variable-set" in types; assert x["bindingCount"]==len(x["objectBindings"])+1; assert all(req["data"]["session_id"]=="session-core-1" for req in x["coreRequests"])
...
def test_service_token_policy_applies_to_v660_routes(monkeypatch):
 monkeypatch.setenv("SCWB_REQUIRE_SERVICE_TOKEN","true"); monkeypatch.setenv("SCWB_SERVICE_TOKEN","secret-660"); c=client(); assert c.get("/integration/core/unified-runtime/manifest").status_code==401; ok=c.get("/integration/core/unified-runtime/manifest",headers={"X-SC-Service-Token":"secret-660"}); assert ok.status_code==200 and ok.json()["version"]=="7.11.0"

def test_v660_status_public_and_non_dispatching():
 b=client().get("/v660/status").json(); assert b["ok"] is True and b["version"]=="7.11.0"; assert b["twoPhaseSessionBinding"] is True; assert b["automaticCoreDispatch"] is False and b["automaticCorePersistence"] is False

def test_capabilities_advertise_session_bridge():
 c=client().get("/capabilities").json(); assert c["version"]=="7.11.0"; assert c["coreIntegration"]["unifiedResearchSessionBinding"] is True; assert "platform-core-unified-research-session-bridge" in c["capabilities"]
