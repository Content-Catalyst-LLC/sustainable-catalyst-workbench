...
def test_manifest_and_status():
    ...
    assert m['ok'] and m['version']=='10.0.0' and m['capabilities']['derivedProjectTimeline'] is True
    assert m['boundaries']['timelineIsCompetingSourceOfTruth'] is False
    s=c.get('/v850/status').json(); assert s['ok'] and s['version']=='10.0.0' and s['runHistory'] is True
