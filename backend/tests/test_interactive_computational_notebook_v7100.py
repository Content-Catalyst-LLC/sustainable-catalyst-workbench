...
def test_manifest_status_capabilities():
    m=c.get('/notebooks/manifest'); assert m.status_code==200; d=m.json(); assert d['version']=='8.7.0' and d['capabilities']['typedNotebookCells']
    s=c.get('/v7100/status').json(); assert s['ok'] and s['hiddenInterpreterState'] is False
    caps=c.get('/capabilities').json(); assert caps['version']=='8.7.0' and caps['coreIntegration']['interactiveComputationalNotebookRuntime'] is True
