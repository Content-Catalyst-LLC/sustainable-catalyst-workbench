...
def test_manifest_status_capabilities():
    ...
    d=m.json(); assert d['schema']==SCHEMA and d['version']=='8.12.0' and d['capabilities']['paretoFrontierExtraction']
    s=cl.get('/v770/status').json(); assert s['ok'] and s['paretoFrontier'] and s['automaticWinnerSelection'] is False
    caps=cl.get('/capabilities').json(); assert caps['version']=='8.12.0' and caps['coreIntegration']['optimizationDesignSpaceRuntime'] is True
