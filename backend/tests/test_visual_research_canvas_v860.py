...
def test_manifest_status_and_capabilities():
    ...
    assert m['ok'] and m['version']=='9.9.0' and m['release']=='Visual Research Canvas'
    assert m['capabilities']['projectVisualCanvas'] is True and m['boundaries']['canvasIsScientificSourceOfTruth'] is False
    s=c.get('/v860/status').json(); assert s['ok'] and s['version']=='9.9.0' and s['persistentLayout'] is True
