...
def test_v560_backend_route_runtime_and_container_identity():
    ...
    assert 'version="5.6.0"' in main or 'version="5.7.0"' in main or 'version="5.8.0"' in main
    assert 'from app.v560 import router as v560_router' in main
    assert 'app.include_router(v560_router)' in main
    assert 'sustainable-catalyst-workbench:5.6.0' in compose or 'sustainable-catalyst-workbench:5.7.0' in compose or 'sustainable-catalyst-workbench:5.8.0' in compose
...
def test_v560_wordpress_contract_and_studio_registration():
    ...
    assert 'Version: 5.6.0' in main or 'Version: 5.7.0' in main or 'Version: 5.8.0' in main
    assert "define('SCWB_VERSION', '5.6.0')" in main or "define('SCWB_VERSION', '5.7.0')" in main or "define('SCWB_VERSION', '5.8.0')" in main
    ...
    assert "const VERSION = '5.6.0'" in primary or "const VERSION = '5.7.0'" in primary or "const VERSION = '5.8.0'" in primary
