...
$ok=strpos($main,'Version: 6.0.0')!==false
    && strpos($main,"define('SCWB_VERSION', '6.0.0')")!==false
    && strpos($main,'SCWB_V600_PLUGIN_FILE')!==false
    && strpos($php,"const VERSION = '6.0.0'")!==false
    && strpos($php,'sc_workbench_computational_project')!==false
    && strpos($primary,"const VERSION = '6.0.0'")!==false
    && strpos($primary,'data-scwb-version="6.0.0"')!==false
    && strpos($catalog,"'computational-project' => array")!==false;
if(!$ok){fwrite(STDERR,"Workbench v6.0.0 activation audit failed.\n");exit(1);}echo "Workbench v6.0.0 activation audit passed.\n";
