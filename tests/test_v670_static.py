...
def test_v670_release_identity_and_router_registration():
    assert 'APP_VERSION = "6.7.0"' in (ROOT/'backend/app/release.py').read_text() or 'APP_VERSION = "6.8.0"' in (ROOT/'backend/app/release.py').read_text()
    ...
    compose=(ROOT/'compose.yml').read_text(); assert ('sustainable-catalyst-workbench:6.7.0' in compose or 'sustainable-catalyst-workbench:6.8.0' in compose) and ("d.get('version')=='6.7.0'" in compose or "d.get('version')=='6.8.0'" in compose)
...
def test_v670_exact_core_lineage_contract_and_paths():
    ...
    for path in ('/v1/research/computation-lineage/executions','/inputs','/parameters','/assumptions','/environments','/steps','/outputs','/research-bindings','/dependencies','/verifications','/revisions','/snapshots'):
        assert path in s
...
def test_v670_wordpress_identity_and_status_bridge():
    ...
    assert ('Version: 6.7.0' in main or 'Version: 6.8.0' in main) and ("define('SCWB_VERSION', '6.7.0')" in main or "define('SCWB_VERSION', '6.8.0')" in main) and 'scwb-v670-computation-execution-lineage.php' in main
