...
def test_v740_release_identity_router_compose():
    assert 'APP_VERSION = "7.6.0"' in (ROOT/'backend/app/release.py').read_text()
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="7.6.0"' in main
...
def test_v740_wordpress_identity_and_status_surface():
    ...
    assert 'Version: 7.6.0' in main and "define('SCWB_VERSION', '7.6.0')" in main
