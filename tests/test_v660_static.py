...
def test_v660_release_identity_and_router_registration():
    release=(ROOT/'backend/app/release.py').read_text(); assert 'APP_VERSION = "6.10.0"' in release
    ...
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:6.10.0' in compose and "d.get('version')=='6.10.0'" in compose
...
def test_v660_wordpress_identity_and_status_bridge():
    ...
    assert 'Version: 6.10.0' in main and "define('SCWB_VERSION', '6.10.0')" in main and 'scwb-v660-unified-research-session-bridge.php' in main
