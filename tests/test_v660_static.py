...
def test_v660_release_identity_and_router_registration():
 assert 'APP_VERSION = "6.6.0"' in (ROOT/'backend/app/release.py').read_text() or 'APP_VERSION = "6.7.0"' in (ROOT/'backend/app/release.py').read_text() or 'APP_VERSION = "6.8.0"' in (ROOT/'backend/app/release.py').read_text(); main=(ROOT/'backend/app/main.py').read_text(); assert 'from app.v660 import router as v660_router' in main and 'app.include_router(v660_router)' in main; compose=(ROOT/'compose.yml').read_text(); assert ('sustainable-catalyst-workbench:6.6.0' in compose or 'sustainable-catalyst-workbench:6.7.0' in compose or 'sustainable-catalyst-workbench:6.8.0' in compose) and ("d.get('version')=='6.6.0'" in compose or "d.get('version')=='6.7.0'" in compose or "d.get('version')=='6.8.0'" in compose)
...
def test_v660_wordpress_identity_and_status_bridge():
 p=ROOT/'wordpress-plugin/sustainable-catalyst-workbench'; main=(p/'sustainable-catalyst-workbench.php').read_text(); inc=(p/'includes/scwb-v660-unified-research-session-bridge.php').read_text(); assert ('Version: 6.6.0' in main or 'Version: 6.7.0' in main or 'Version: 6.8.0' in main) and ("define('SCWB_VERSION', '6.6.0')" in main or "define('SCWB_VERSION', '6.7.0')" in main or "define('SCWB_VERSION', '6.8.0')" in main) and 'scwb-v660-unified-research-session-bridge.php' in main; assert '/v660/status' in inc and 'sc_workbench_core_session_status' in inc
