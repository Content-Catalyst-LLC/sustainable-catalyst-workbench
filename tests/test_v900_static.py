...
def test_v900_release_identity_and_routes():
    ...
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="9.9.0"' in main and 'from app.v900 import router as v900_router' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:9.9.0' in compose and "d.get('version')=='9.9.0'" in compose and '127.0.0.1:8088:8088' in compose
...
def test_v900_wordpress_and_boundary_contract():
    main=(ROOT/'wordpress-plugin/sustainable-catalyst-workbench/sustainable-catalyst-workbench.php').read_text(); assert 'Version: 9.9.0' in main and "define('SCWB_VERSION', '9.9.0')" in main and 'SCWB_DIR' not in main and 'scwb-v900-unified-scientific-study-composer.php' in main
