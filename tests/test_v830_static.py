...
def test_v830_identity_router_compose():
    ...
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="8.3.0"' in main and 'from app.v830 import router as v830_router' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:8.3.0' in compose and "d.get('version')=='8.3.0'" in compose and './data:/data' in compose
...
def test_v830_wordpress():
    ...
    assert 'Version: 8.3.0' in main and "define('SCWB_VERSION', '8.3.0')" in main and 'SCWB_DIR' not in main
