...
def test_v860_identity_router_compose():
    ...
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="9.11.0"' in main and 'from app.v860 import router as v860_router' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:9.11.0' in compose and "d.get('version')=='9.11.0'" in compose and './data:/data' in compose
...
def test_v860_wordpress():
    ...
    assert 'Version: 9.11.0' in main and "define('SCWB_VERSION', '9.11.0')" in main and 'SCWB_DIR' not in main
