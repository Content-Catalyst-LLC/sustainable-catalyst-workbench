...
def test_v680_release_identity_and_routes():
    assert 'APP_VERSION = "6.8.0"' in (ROOT/'backend/app/release.py').read_text()
    ...
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:6.8.0' in compose and "d.get('version')=='6.8.0'" in compose
...
def test_v680_wordpress_bridge_and_shortcode():
    ...
    assert 'Version: 6.8.0' in main and "define('SCWB_VERSION', '6.8.0')" in main
