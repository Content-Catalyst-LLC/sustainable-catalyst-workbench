...
def test_v870_identity_router_compose():
    ...
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="8.7.0"' in main and 'from app.v870 import router as v870_router' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:8.7.0' in compose and "d.get('version')=='8.7.0'" in compose and './data:/data' in compose
...
def test_v870_capabilities_docs():
    ...
    assert (ROOT/'workbench-v8.7.0.env.example').exists()
