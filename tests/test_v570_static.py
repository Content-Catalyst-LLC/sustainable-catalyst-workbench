...
def test_v570_backend_route_runtime_and_container_identity():
    ...
    assert 'version="5.7.0"' in main or 'version="5.8.0"' in main or 'version="5.9.0"' in main
    assert 'from app.v570 import router as v570_router' in main
    assert 'app.include_router(v570_router)' in main
    assert 'sustainable-catalyst-workbench:5.7.0' in compose or 'sustainable-catalyst-workbench:5.8.0' in compose or 'sustainable-catalyst-workbench:5.9.0' in compose
...
def test_v570_wordpress_contract_and_studio_registration():
    ...
    assert 'Version: 5.7.0' in main or 'Version: 5.8.0' in main or 'Version: 5.9.0' in main
    assert "define('SCWB_VERSION', '5.7.0')" in main or "define('SCWB_VERSION', '5.8.0')" in main or "define('SCWB_VERSION', '5.9.0')" in main
    ...
    assert "const VERSION = '5.7.0'" in primary or "const VERSION = '5.8.0'" in primary or "const VERSION = '5.9.0'" in primary
    assert 'data-scwb-version="5.7.0"' in primary or 'data-scwb-version="5.8.0"' in primary or 'data-scwb-version="5.9.0"' in primary
