...
def test_v880_release_identity_and_routes():
 assert 'APP_VERSION = "8.10.0"' in (ROOT/'backend/app/release.py').read_text()
 main=(ROOT/'backend/app/main.py').read_text(); assert 'version="8.10.0"' in main and 'from app.v880 import router as v880_router' in main
 compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:8.10.0' in compose and "d.get('version')=='8.10.0'" in compose
def test_v880_artifacts_and_wp():
 ...
 main=(ROOT/'wordpress-plugin/sustainable-catalyst-workbench/sustainable-catalyst-workbench.php').read_text(); assert 'Version: 8.10.0' in main and "define('SCWB_VERSION', '8.10.0')" in main and 'scwb-v880-comparative-experiment-model-analysis.php' in main and 'SCWB_DIR' not in main
