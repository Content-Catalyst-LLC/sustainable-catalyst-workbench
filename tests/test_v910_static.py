...
def test_release_identity_and_routes():
    ...
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:9.9.0' in compose
...
def test_wordpress_and_deployment_contracts():
    ...
    assert 'Version: 9.9.0' in main and inc.exists() and 'sc_workbench_research_protocol_builder' in inc.read_text()
