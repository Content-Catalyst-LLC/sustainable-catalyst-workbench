...
def test_v640_release_identity_and_files():
    ...
    assert 'APP_VERSION = "6.4.0"' in release or 'APP_VERSION = "6.5.0"' in release or 'APP_VERSION = "6.6.0"' in release
    assert "from app.v640 import router as v640_router" in main
    assert "app.include_router(v640_router)" in main
    assert "Version: 6.4.0" in plugin or "Version: 6.5.0" in plugin or 'Version: 6.6.0' in plugin
    assert "define('SCWB_VERSION', '6.4.0')" in plugin or "define('SCWB_VERSION', '6.5.0')" in plugin or "define('SCWB_VERSION', '6.6.0')" in plugin
    assert "scwb-v640-platform-core-connectivity.php" in plugin
    assert "sustainable-catalyst-workbench:6.4.0" in compose or "sustainable-catalyst-workbench:6.5.0" in compose or 'sustainable-catalyst-workbench:6.6.0' in compose
