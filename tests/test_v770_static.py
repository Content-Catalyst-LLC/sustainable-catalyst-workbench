...
def test_v770_identity_router_compose():
    assert 'APP_VERSION = "7.7.0"' in (ROOT/'backend/app/release.py').read_text()
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="7.7.0"' in main
    assert 'from app.v770 import router as v770_router' in main and 'app.include_router(v770_router)' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:7.7.0' in compose and "d.get('version')=='7.7.0'" in compose
...
def test_v770_wordpress():
    ...
    assert 'Version: 7.7.0' in main and "define('SCWB_VERSION', '7.7.0')" in main and 'SCWB_DIR' not in main
