...
def test_v790_identity_router_compose():
    ...
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="8.6.0"' in main
    assert 'from app.v790 import router as v790_router' in main and 'app.include_router(v790_router)' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:8.6.0' in compose and "d.get('version')=='8.6.0'" in compose
