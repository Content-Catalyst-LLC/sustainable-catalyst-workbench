...
def test_v820_identity_router_compose():
    assert 'APP_VERSION = "8.6.0"' in (ROOT/'backend/app/release.py').read_text()
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="8.6.0"' in main and 'from app.v820 import router as v820_router' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:8.6.0' in compose and "d.get('version')=='8.6.0'" in compose and './data:/data' in compose
...
def test_v820_wordpress():
    ...
    assert 'Version: 8.6.0' in main and "define('SCWB_VERSION', '8.6.0')" in main and 'SCWB_DIR' not in main
