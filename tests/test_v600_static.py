...
def test_v600_backend_route_runtime_and_container_identity():
    ...
    assert 'version="6.0.0"' in main
    assert 'from app.v600 import router as v600_router' in main
    assert 'app.include_router(v600_router)' in main
    assert 'sustainable-catalyst-workbench:6.0.0' in compose
    assert 'VERSION = "6.0.0"' in backend
...
def test_v600_wordpress_contract_and_studio_registration():
    ...
    assert 'Version: 6.0.0' in main
    assert "define('SCWB_VERSION', '6.0.0')" in main
    assert 'SCWB_V600_PLUGIN_FILE' in main
    assert "const VERSION = '6.0.0'" in php
    assert 'sc_workbench_computational_project' in php
    assert "'computational-project' => array" in catalog
    assert "const VERSION = '6.0.0'" in primary
    assert 'data-scwb-version="6.0.0"' in primary
...
def test_v600_browser_runtime_contract_and_viewport_safety():
    ...
    for marker in ["VERSION='6.0.0'", 'projectBuild', 'variablesResolve', 'linksValidate', 'historyBuild', 'exportBuild', 'handoffBuild', 'localStorage']:
        assert marker in js
