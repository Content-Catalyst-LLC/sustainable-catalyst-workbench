...
def test_v650_release_identity_and_routes_are_registered():
    ...
    assert 'APP_VERSION = "6.5.0"' in release or 'APP_VERSION = "6.6.0"' in release or 'APP_VERSION = "6.7.0"' in release or 'APP_VERSION = "6.8.0"' in release or 'APP_VERSION = "8.8.0"' in release
    assert 'from app.v650 import router as v650_router' in main
    assert 'app.include_router(v650_router)' in main
    assert 'sustainable-catalyst-workbench:6.5.0' in compose or 'sustainable-catalyst-workbench:6.6.0' in compose or 'sustainable-catalyst-workbench:6.7.0' in compose or 'sustainable-catalyst-workbench:6.8.0' in compose or 'sustainable-catalyst-workbench:8.8.0' in compose
    assert "d.get('version')=='6.5.0'" in compose or "d.get('version')=='6.6.0'" in compose or "d.get('version')=='6.7.0'" in compose or "d.get('version')=='6.8.0'" in compose or "d.get('version')=='8.8.0'" in compose
...
def test_wordpress_release_identity_and_adapter_bridge():
    ...
    assert 'Version: 6.5.0' in main or 'Version: 6.6.0' in main or 'Version: 6.7.0' in main or 'Version: 6.8.0' in main or 'Version: 8.8.0' in main
    assert "define('SCWB_VERSION', '6.5.0')" in main or "define('SCWB_VERSION', '6.6.0')" in main or "define('SCWB_VERSION', '6.7.0')" in main or "define('SCWB_VERSION', '6.8.0')" in main or "define('SCWB_VERSION', '8.8.0')" in main
