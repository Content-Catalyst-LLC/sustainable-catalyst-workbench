...
def test_v7100_identity_router_compose():
    ...
    main=(ROOT/'backend/app/main.py').read_text(); assert 'version="8.2.0"' in main
    assert 'from app.v7100 import router as v7100_router' in main and 'app.include_router(v7100_router)' in main
    compose=(ROOT/'compose.yml').read_text(); assert 'sustainable-catalyst-workbench:8.2.0' in compose and "d.get('version')=='8.2.0'" in compose
