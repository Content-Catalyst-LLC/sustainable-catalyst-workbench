(function(){
  ...
  function initWorkbench(el){
    ...
    el.addEventListener('click', ev=>{ const action=ev.target && ev.target.dataset && ev.target.dataset.scwbExport; if(!action) return; const out=ev.target.closest('.scwb-output') || el.querySelector('.scwb-output'); const data=out && out.__scwbLastResult ? out.__scwbLastResult : null; if(action==='json' && data){ downloadBlob(safeName(data.tool||'workbench-result')+'.json', new Blob([JSON.stringify(data,null,2)], {type:'application/json'})); } if(action==='pdf'){ const res=ev.target.closest('.scwb-result'); openPdfReport(data||{}, res ? res.outerHTML : ''); } if(action==='svg'){ downloadSvg(ev.target.closest('.scwb-graph')); } if(action==='png'){ downloadPng(ev.target.closest('.scwb-graph')); } });
