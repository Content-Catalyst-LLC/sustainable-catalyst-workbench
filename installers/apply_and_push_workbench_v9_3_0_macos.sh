...
chmod +x "$TARGET/scripts/test_v930_release.sh" "$TARGET/deploy/contabo/upgrade_workbench_backend_v9_3_0_contabo.sh" "$TARGET/installers/apply_and_push_workbench_v9_3_0_macos.sh"
...
PYCHECK
  then
    ...
    VENV="${TMPDIR:-/tmp}/scwb-v930-release-venv"
...
echo "=== VERIFYING CANONICAL v9.3.0 IDENTITY BEFORE TESTS ==="
PYTHONPATH="$TARGET/backend" "$TEST_PYTHON" - <<'PYIDENT'
import app.release as r
print('release module:',r.__file__); print('APP_VERSION:',r.APP_VERSION); assert r.APP_VERSION=='9.3.0'
PYIDENT

echo "=== VALIDATING RELEASE ==="
SCWB_TEST_PYTHON="$TEST_PYTHON" "$TARGET/scripts/test_v930_release.sh"
git add -A
if git diff --cached --quiet; then
  echo "No new changes to commit; v$VERSION patch may already be committed."
else
  git commit -m "Workbench v9.3.0 Statistical Analysis & Diagnostic Workspace"
fi
...
if git rev-parse "v$VERSION" >/dev/null 2>&1; then
  ...
  git tag -a "v$VERSION" -m "Workbench v$VERSION — Statistical Analysis & Diagnostic Workspace"
fi
