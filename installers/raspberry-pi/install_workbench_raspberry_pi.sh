...
case "$ARCH" in
  aarch64|armv7l) ;;
  *) echo "WARNING: $ARCH is not a standard Raspberry Pi OS architecture." >&2 ;;
esac
...
mkdir -p "$BIN_DIR"
"$ROOT/installers/common/install_workbench_common.sh" raspberry-pi "$ROOT" "$TARGET" "$LAUNCHER"
...
Comment=Offline Workbench for Raspberry Pi OS
Exec=$LAUNCHER
Terminal=true
