...
chmod +x "$TARGET/scripts/test_v990_release.sh" "$TARGET/deploy/contabo/upgrade_workbench_backend_v9_9_0_contabo.sh" "$TARGET/installers/apply_and_push_workbench_v9_9_0_macos.sh"
...
PYCHECK
  ...
  else VENV="${TMPDIR:-/tmp}/scwb-v990-release-venv"; echo "=== BOOTSTRAPPING ISOLATED RELEASE TEST ENVIRONMENT ==="; rm -rf "$VENV"; python3 -m venv "$VENV"; "$VENV/bin/python" -m pip install -U pip; "$VENV/bin/python" -m pip install -r "$TARGET/backend/requirements.txt"; TEST_PYTHON="$VENV/bin/python"; fi
fi
echo "=== VERIFYING CANONICAL v9.9.0 IDENTITY BEFORE TESTS ==="
PYTHONPATH="$TARGET/backend" "$TEST_PYTHON" - <<'PYIDENT'
import app.release as r
print('release module:',r.__file__); print('APP_VERSION:',r.APP_VERSION); assert r.APP_VERSION=='9.9.0'
PYIDENT
echo "=== VALIDATING RELEASE ==="; SCWB_TEST_PYTHON="$TEST_PYTHON" "$TARGET/scripts/test_v990_release.sh"
find "$TARGET/backend" -type d -name '__pycache__' -prune -exec rm -rf {} + 2>/dev/null || true
find "$TARGET" -type d -name '.pytest_cache' -prune -exec rm -rf {} + 2>/dev/null || true
find "$TARGET" -type f -name '*.pyc' -delete 2>/dev/null || true
git add -A
if git diff --cached --quiet; then echo "No new changes to commit; v$VERSION patch may already be committed."; else git commit -m "Workbench v9.9.0 Research Package Exchange & Portability"; fi
echo "=== PUSHING $BRANCH ==="; git push "$REMOTE" "$BRANCH"
if git rev-parse "v$VERSION" >/dev/null 2>&1; then echo "Tag v$VERSION already exists locally."; else git tag -a "v$VERSION" -m "Workbench v$VERSION — Research Package Exchange & Portability"; fi
