...
chmod +x "$TARGET/scripts/test_v890_release.sh" "$TARGET/deploy/contabo/upgrade_workbench_backend_v8_9_0_contabo.sh" "$TARGET/installers/apply_and_push_workbench_v8_9_0_macos.sh"
...
PYCHECK
  then
    ...
    VENV="${TMPDIR:-/tmp}/scwb-v890-release-venv"
...
SCWB_TEST_PYTHON="$TEST_PYTHON" "$TARGET/scripts/test_v890_release.sh"
git add -A
if git diff --cached --quiet; then
  echo "No new changes to commit; v$VERSION patch may already be committed."
else
  git commit -m "Workbench v8.9.0 Interactive Scientific Figure & Visualization Composer"
fi
...
if git rev-parse "v$VERSION" >/dev/null 2>&1; then
  ...
  git tag -a "v$VERSION" -m "Workbench v$VERSION — Interactive Scientific Figure & Visualization Composer"
fi
