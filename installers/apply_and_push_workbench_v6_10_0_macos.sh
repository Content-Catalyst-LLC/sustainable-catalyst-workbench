...
if [[ -n "$DIRTY" ]]; then
 if [[ -f "$TARGET/backend/app/release.py" ]] && grep -q 'APP_VERSION = "6.10.0"' "$TARGET/backend/app/release.py" && [[ -f "$TARGET/backend/app/v6100.py" ]] && [[ -f "$TARGET/scripts/test_v6100_release.sh" ]]; then RESUME=1; echo "=== RESUMING PARTIALLY APPLIED WORKBENCH v$VERSION ==="
...
chmod +x "$TARGET/scripts/test_v6100_release.sh" "$TARGET/deploy/contabo/upgrade_workbench_backend_v6_10_0_contabo.sh" "$TARGET/installers/apply_and_push_workbench_v6_10_0_macos.sh"
TEST_PYTHON="${SCWB_TEST_PYTHON:-python3}"; VENV_DIR="${TMPDIR:-/tmp}/scwb-v6100-release-venv"
...
SCWB_TEST_PYTHON="$TEST_PYTHON" "$TARGET/scripts/test_v6100_release.sh"
echo "=== COMMITTING RELEASE ==="
while IFS= read -r rel; do git -C "$TARGET" add -- "$rel"; done < <(cd "$PATCH_DIR" && find . -type f -print | sed 's#^./##' | sort)
if git -C "$TARGET" diff --cached --quiet; then echo "No new changes to commit; v$VERSION patch may already be committed."; else git -C "$TARGET" commit -m "Workbench v6.10.0 predictive intelligence runtime integration"; fi
echo "=== PUSHING $BRANCH ==="; git -C "$TARGET" push "$REMOTE" "$BRANCH"
if git -C "$TARGET" rev-parse "v$VERSION" >/dev/null 2>&1; then echo "Tag v$VERSION already exists; leaving it unchanged."; else git -C "$TARGET" tag -a "v$VERSION" -m "Workbench v$VERSION — Predictive Intelligence Runtime Integration"; fi
