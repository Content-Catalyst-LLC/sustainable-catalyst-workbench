...
echo "Testing Sustainable Catalyst Workbench v5.0.0"
grep -q 'Version: 5.0.0' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '5.0.0')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="5.0.0"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v500_plugin_activation.php"
...
if [[ "${SCWB_SKIP_GO_TESTS:-0}" != "1" ]] && command -v go >/dev/null 2>&1; then (cd "$ROOT/runner-go" && GOTOOLCHAIN=local go test ./... && GOTOOLCHAIN=local go vet ./...); elif [[ "${SCWB_SKIP_GO_TESTS:-0}" == "1" ]]; then echo "Go compatibility checks skipped by SCWB_SKIP_GO_TESTS=1."; fi
for required in "$PLUGIN/includes/scwb-v500-integrated-platform.php" "$PLUGIN/assets/js/sc-workbench-v500.js" "$PLUGIN/assets/css/sc-workbench-v500.css" "$ROOT/backend/app/v500.py" "$ROOT/docs/V500_SECURITY_BOUNDARY.md" "$ROOT/docs/V500_INTEGRATED_PLATFORM.md" "$ROOT/examples/v500-integrated-platform-fixture.json" "$ROOT/offline/package-manifest.json"; do [[ -s "$required" ]] || { echo "Missing v5.0.0 artifact: $required" >&2; exit 1; }; done
grep -q 'automaticWorkflowExecutionAuthorized.*False' "$ROOT/backend/app/v500.py"
grep -q 'automaticPublicationAuthorized.*False' "$ROOT/backend/app/v500.py"
grep -q 'automaticCertificationAuthorized.*False' "$ROOT/backend/app/v500.py"
grep -q 'automaticDeviceExecutionAuthorized.*False' "$ROOT/backend/app/v500.py"
grep -q 'remoteShellAuthorized.*False' "$ROOT/backend/app/v500.py"
grep -q 'arbitraryCommandExecutionAuthorized.*False' "$ROOT/backend/app/v500.py"
grep -q '"version": "5.0.0"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v500.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v5.0.0 release checks passed."
