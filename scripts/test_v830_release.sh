...
export SCWB_RESEARCH_ENVIRONMENT_STORE="${SCWB_RESEARCH_ENVIRONMENT_STORE:-${TMPDIR:-/tmp}/scwb-v830-release-store}"
rm -rf "$SCWB_RESEARCH_ENVIRONMENT_STORE"
echo "=== Workbench v8.3.0 release validation ==="
"$PY" -m pytest -q \
...
"$PY" -m pytest -q \
  ...
  "$ROOT/tests/test_v800_static.py" "$ROOT/tests/test_v810_static.py" "$ROOT/tests/test_v820_static.py" "$ROOT/tests/test_v830_static.py"
...
php -l "$ROOT/wordpress-plugin/sustainable-catalyst-workbench/includes/scwb-v830-research-asset-artifact-registry.php" >/dev/null
...
os.environ['SCWB_RESEARCH_ENVIRONMENT_STORE']=tempfile.mkdtemp(prefix='scwb-v830-release-')
c=TestClient(app)
assert c.get('/health').json()['version']=='8.3.0'
...
spec={'environmentKey':'release-v830-env','title':'Release Environment','projectEntityId':'release-v830-project','components':[{'componentKey':'data','componentType':'data-workspace','payload':{'workspaceHash':'release-data'},'metadata':{'title':'Release dataset'}}]}
...
ws=c.post('/research-projects/build',json={'project':{'projectKey':'release-v830-project','title':'Release Project','activeEnvironmentKey':'release-v830-env','activeEnvironmentRevision':1}}); assert ws.status_code==200,ws.text
saved=c.post('/research-projects/save',json={'workspace':ws.json(),'expectedProjectRevision':0,'reason':'release-check'}); assert saved.status_code==200,saved.text
idx=c.post('/research-assets/index/project',json={'projectKey':'release-v830-project'}); assert idx.status_code==200 and idx.json()['indexedCount']==1,idx.text
search=c.get('/research-assets/search',params={'project_key':'release-v830-project','asset_type':'dataset'}); assert search.status_code==200 and search.json()['resultCount']==1,search.text
print('PASS: Workbench v8.3.0 research asset and artifact registry endpoints assembled')
PYVERIFY
echo "PASS: Workbench v8.3.0 release checks passed."
