...
echo "Testing Sustainable Catalyst Workbench v4.3.0"
grep -q 'Version: 4.3.0' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '4.3.0')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="4.3.0"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v430_plugin_activation.php"
...
for required in "$PLUGIN/includes/scwb-v430-data-pipelines.php" "$PLUGIN/assets/js/sc-workbench-v430.js" "$PLUGIN/assets/css/sc-workbench-v430.css" "$ROOT/backend/app/v430.py" "$ROOT/docs/V430_SECURITY_BOUNDARY.md" "$ROOT/examples/v430-data-pipeline-fixture.json" "$ROOT/offline/package-manifest.json"; do [[ -s "$required" ]] || { echo "Missing v4.3.0 artifact: $required" >&2; exit 1; }; done
grep -q 'automaticNetworkFetchAuthorized.*False' "$ROOT/backend/app/v430.py"
grep -q 'automaticCredentialStorageAuthorized.*False' "$ROOT/backend/app/v430.py"
grep -q 'automaticCloudUploadAuthorized.*False' "$ROOT/backend/app/v430.py"
grep -q 'automaticPublicationAuthorized.*False' "$ROOT/backend/app/v430.py"
grep -q 'automaticDeletionAuthorized.*False' "$ROOT/backend/app/v430.py"
grep -q '"version": "4.3.0"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v430.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v4.3.0 release checks passed."
