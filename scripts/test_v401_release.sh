...
echo "Testing Sustainable Catalyst Workbench v4.0.1"
grep -q 'Version: 4.0.1' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '4.0.1')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="4.0.1"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v401_plugin_activation.php"
...
if command -v go >/dev/null 2>&1; then (cd "$ROOT/runner-go" && GOTOOLCHAIN=local go test ./... && GOTOOLCHAIN=local go vet ./...); fi
for required in \
  "$PLUGIN/includes/scwb-v401-connected-reliability.php" \
  "$PLUGIN/assets/js/sc-workbench-v401.js" \
  "$PLUGIN/assets/css/sc-workbench-v401.css" \
  "$ROOT/backend/app/v401.py" \
  "$ROOT/docs/V401_SECURITY_BOUNDARY.md" \
  "$ROOT/examples/v401-connected-reliability-fixture.json" \
  "$ROOT/offline/package-manifest.json"; do
  [[ -s "$required" ]] || { echo "Missing v4.0.1 artifact: $required" >&2; exit 1; }
done
grep -q 'automaticRepairAuthorized.*False' "$ROOT/backend/app/v401.py"
grep -q 'automaticDestructiveActionAuthorized.*False' "$ROOT/backend/app/v401.py"
grep -q '"version": "4.0.1"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v401.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v4.0.1 release checks passed."
