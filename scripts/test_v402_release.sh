...
echo "Testing Sustainable Catalyst Workbench v4.0.2"
grep -q 'Version: 4.0.2' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '4.0.2')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="4.0.2"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v402_plugin_activation.php"
...
for required in \
  "$PLUGIN/includes/scwb-v402-graph-sync-recovery.php" \
  "$PLUGIN/assets/js/sc-workbench-v402.js" \
  "$PLUGIN/assets/css/sc-workbench-v402.css" \
  "$ROOT/backend/app/v402.py" \
  "$ROOT/docs/V402_SECURITY_BOUNDARY.md" \
  "$ROOT/examples/v402-graph-sync-recovery-fixture.json" \
  "$ROOT/offline/package-manifest.json"; do
  [[ -s "$required" ]] || { echo "Missing v4.0.2 artifact: $required" >&2; exit 1; }
done
grep -q 'automaticExecutionAuthorized.*False' "$ROOT/backend/app/v402.py"
grep -q 'automaticRollbackAuthorized.*False' "$ROOT/backend/app/v402.py"
grep -q 'automaticDeletionAuthorized.*False' "$ROOT/backend/app/v402.py"
grep -q '"version": "4.0.2"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v402.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v4.0.2 release checks passed."
