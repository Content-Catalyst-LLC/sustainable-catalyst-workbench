...
echo "Testing Sustainable Catalyst Workbench v4.4.0"
grep -q 'Version: 4.4.0' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '4.4.0')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="4.4.0"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v440_plugin_activation.php"
...
for required in "$PLUGIN/includes/scwb-v440-evaluation-laboratory.php" "$PLUGIN/assets/js/sc-workbench-v440.js" "$PLUGIN/assets/css/sc-workbench-v440.css" "$ROOT/backend/app/v440.py" "$ROOT/docs/V440_SECURITY_BOUNDARY.md" "$ROOT/examples/v440-evaluation-lab-fixture.json" "$ROOT/offline/package-manifest.json"; do [[ -s "$required" ]] || { echo "Missing v4.4.0 artifact: $required" >&2; exit 1; }; done
grep -q 'automaticExperimentExecutionAuthorized.*False' "$ROOT/backend/app/v440.py"
grep -q 'automaticLeaderboardPublicationAuthorized.*False' "$ROOT/backend/app/v440.py"
grep -q 'automaticBaselineReplacementAuthorized.*False' "$ROOT/backend/app/v440.py"
grep -q 'automaticReleaseApprovalAuthorized.*False' "$ROOT/backend/app/v440.py"
grep -q 'automaticCertificationAuthorized.*False' "$ROOT/backend/app/v440.py"
grep -q '"version": "4.4.0"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v440.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v4.4.0 release checks passed."
