#!/usr/bin/env python3
"""Merge Workbench v2.8.0 into an existing v2.7.0 repository checkout."""
...
VERSION = "2.8.0"
RELEASE_TITLE = "Experiment Automation and Reproducible Workflow Studio"
...
def patch(repo: Path, overlay: Path) -> None:
    ...
    prerequisite = plugin / "includes/scwb-v270-visualization-dashboard.php"
    if not prerequisite.exists():
        raise SystemExit("Workbench v2.7.0 is required before v2.8.0.")
    ...
    if "SCWB_V280_PLUGIN_FILE" not in plugin_text:
    ...
    if "v280_router" not in backend_text:
    ...
    shutil.copy2(overlay / "scripts/test_v280_release.sh", repo / "scripts/test_v280_release.sh")
    (repo / "scripts/test_v280_release.sh").chmod(0o755)
    shutil.copy2(overlay / "docs/V280_RELEASE_NOTES.md", repo / "V280_RELEASE_NOTES.md")

    readme = repo / "README.md"
    if readme.exists():
        text = re.sub(r"(?m)^# Sustainable Catalyst(?: Prototyping)? Workbench v[^\s]+", "# Sustainable Catalyst Prototyping Workbench v2.8.0", readme.read_text(), count=1)
        heading = "## Version 2.8.0 — Experiment Automation and Reproducible Workflow Studio"
        if heading not in text:
            text = text.rstrip() + "\n\n" + heading + "\n\nAdds structured protocols, dependency-aware workflows, schedule planning, version manifests, automatic checkpoints, reproducibility comparison, deviation records, and exportable experiment bundles.\n"
        readme.write_text(text)

    required = [
        plugin / "includes/scwb-v280-experiment-automation.php",
        plugin / "assets/js/sc-workbench-v280.js",
        plugin / "assets/css/sc-workbench-v280.css",
        app / "v280.py",
        repo / "runner-go/internal/runner/version.go",
    ]
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise SystemExit("v2.8.0 merge incomplete:\n" + "\n".join(missing))
