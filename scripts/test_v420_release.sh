...
echo "Testing Sustainable Catalyst Workbench v4.2.0"
grep -q 'Version: 4.2.0' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '4.2.0')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="4.2.0"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v420_plugin_activation.php"
...
elif [[ "${SCWB_SKIP_GO_TESTS:-0}" == "1" ]]; then
  echo "Go compatibility checks skipped by SCWB_SKIP_GO_TESTS=1."
fi
for required in \
  "$PLUGIN/includes/scwb-v420-guided-projects.php" \
  "$PLUGIN/assets/js/sc-workbench-v420.js" \
  "$PLUGIN/assets/css/sc-workbench-v420.css" \
  "$ROOT/backend/app/v420.py" \
  "$ROOT/docs/V420_SECURITY_BOUNDARY.md" \
  "$ROOT/examples/v420-guided-project-fixture.json" \
  "$ROOT/offline/package-manifest.json"; do
  [[ -s "$required" ]] || { echo "Missing v4.2.0 artifact: $required" >&2; exit 1; }
done
grep -q 'automaticExperimentExecutionAuthorized.*False' "$ROOT/backend/app/v420.py"
grep -q 'automaticDeviceControlAuthorized.*False' "$ROOT/backend/app/v420.py"
grep -q 'automaticPublicationAuthorized.*False' "$ROOT/backend/app/v420.py"
grep -q 'automaticCertificationAuthorized.*False' "$ROOT/backend/app/v420.py"
grep -q '"version": "4.2.0"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v420.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v4.2.0 release checks passed."
