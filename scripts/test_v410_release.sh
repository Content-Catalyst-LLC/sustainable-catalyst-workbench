...
echo "Testing Sustainable Catalyst Workbench v4.1.0"
grep -q 'Version: 4.1.0' "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q "define('SCWB_VERSION', '4.1.0')" "$PLUGIN/sustainable-catalyst-workbench.php"
grep -q 'version="4.1.0"' "$ROOT/backend/app/main.py"
...
php "$ROOT/tests/test_v410_plugin_activation.php"
...
for required in \
  "$PLUGIN/includes/scwb-v410-team-workspace.php" \
  "$PLUGIN/assets/js/sc-workbench-v410.js" \
  "$PLUGIN/assets/css/sc-workbench-v410.css" \
  "$ROOT/backend/app/v410.py" \
  "$ROOT/docs/V410_SECURITY_BOUNDARY.md" \
  "$ROOT/examples/v410-team-workspace-fixture.json" \
  "$ROOT/offline/package-manifest.json"; do
  [[ -s "$required" ]] || { echo "Missing v4.1.0 artifact: $required" >&2; exit 1; }
done
grep -q 'automaticInvitationAcceptanceAuthorized.*False' "$ROOT/backend/app/v410.py"
grep -q 'automaticMembershipEscalationAuthorized.*False' "$ROOT/backend/app/v410.py"
grep -q 'automaticProjectDeletionAuthorized.*False' "$ROOT/backend/app/v410.py"
grep -q '"version": "4.1.0"' "$ROOT/offline/package-manifest.json"
grep -q 'backend/app/v410.py' "$ROOT/offline/package-manifest.json"
...
echo "Workbench v4.1.0 release checks passed."
