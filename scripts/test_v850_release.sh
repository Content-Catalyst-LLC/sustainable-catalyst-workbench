...
export SCWB_RESEARCH_ENVIRONMENT_STORE="${SCWB_RESEARCH_ENVIRONMENT_STORE:-${TMPDIR:-/tmp}/scwb-v850-release-store}"
rm -rf "$SCWB_RESEARCH_ENVIRONMENT_STORE"
echo "=== Workbench v8.5.0 release validation ==="
"$PY" -m pytest -q \
  ...
  "$ROOT/backend/tests/test_research_timeline_run_history_v850.py"
"$PY" -m pytest -q \
  ...
  "$ROOT/tests/test_v800_static.py" "$ROOT/tests/test_v810_static.py" "$ROOT/tests/test_v820_static.py" "$ROOT/tests/test_v830_static.py" "$ROOT/tests/test_v840_static.py" "$ROOT/tests/test_v850_static.py"
...
os.environ['SCWB_RESEARCH_ENVIRONMENT_STORE']=tempfile.mkdtemp(prefix='scwb-v850-release-')
c=TestClient(app)
assert c.get('/health').json()['version']=='8.5.0'
m=c.get('/research-timeline/manifest').json(); assert m['ok'] and m['version']=='8.5.0' and m['capabilities']['derivedProjectTimeline'] is True
uid='release-v850'
env=c.post('/research-environment/build',json={'environment':{'environmentKey':uid+'-env','title':'Release Timeline Environment','projectEntityId':uid+'-project','components':[]}}).json()
...
assert c.post('/research-environment/checkpoints/create',json={'environmentKey':uid+'-env','revision':1,'label':'release-baseline'}).status_code==200
ws=c.post('/research-projects/build',json={'project':{'projectKey':uid+'-project','title':'Release Timeline Project','activeEnvironmentKey':uid+'-env','activeEnvironmentRevision':1}}).json()
assert c.post('/research-projects/save',json={'workspace':ws,'expectedProjectRevision':0,'reason':'release-check'}).status_code==200
job=c.post('/execution-console/jobs/prepare',json={'projectKey':uid+'-project','runtimeKind':'solver','request':{'problemKind':'root','problem':{'expression':'x**2-4','variable':'x','bracket':[0,3]},'solverKey':'root.brentq'}}); assert job.status_code==200,job.text
...
t=c.get('/research-timeline/'+uid+'-project'); assert t.status_code==200,t.text
assert t.json()['eventCount']>=5 and any(e['eventType']=='execution-completed' for e in t.json()['events'])
lin=c.get('/research-timeline/'+uid+'-project/lineage'); assert lin.status_code==200 and lin.json()['nodeCount']>=5
print('PASS: Workbench v8.5.0 research timeline and run history endpoints assembled')
PYVERIFY
echo "PASS: Workbench v8.5.0 release checks passed."
